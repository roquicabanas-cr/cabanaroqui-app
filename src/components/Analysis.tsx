...
export function Analysis({ 
  ...
  const yearlyStats = useMemo(() => {
    ...
    const totalIncome = yearReservations.reduce((sum, r) => sum + r.netIncome, 0);
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const income = monthReservations.reduce((sum, r) => sum + r.netIncome, 0);
  ...
  const platformData = useMemo(() => {
    ...
    reservations
      ...
      .forEach(r => {
        ...
        distribution[r.platform].income += r.netIncome;
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#F4F4F4]">Análisis Financiero</h2>
          <p className="text-[#F4F4F4]/60">Balances, estadísticas y reportes detallados</p>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={selectedYear}
            onChange={(e) => setSelectedYear(Number(e.target.value))}
            className="bg-[#F4F4F4]/5 border border-[#E6A800]/20 text-[#F4F4F4] rounded-lg px-3 py-2"
          >
            {years.map(year => (
              <option key={year} value={year} className="bg-[#0B0B0B]">{year}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-[#0B0B0B] border-[#E6A800]/20">
          <CardContent className="p-4">
            <p className="text-sm text-[#F4F4F4]/60">Ingresos {selectedYear}</p>
            <p className="text-xl font-bold text-[#E6A800] mt-1">
        ...
        <Card className="bg-[#0B0B0B] border-[#E6A800]/20">
          <CardContent className="p-4">
            <p className="text-sm text-[#F4F4F4]/60">Gastos {selectedYear}</p>
            <p className="text-xl font-bold text-[#E74C3C] mt-1">
        ...
        <Card className="bg-[#0B0B0B] border-[#E6A800]/20">
          <CardContent className="p-4">
            <p className="text-sm text-[#F4F4F4]/60">Ganancia Neta</p>
            <p className={`text-xl font-bold mt-1 ${yearlyStats.netProfit >= 0 ? 'text-[#2ECC71]' : 'text-[#E74C3C]'}`}>
        ...
        <Card className="bg-[#0B0B0B] border-[#E6A800]/20">
          <CardContent className="p-4">
            <p className="text-sm text-[#F4F4F4]/60">Reservas</p>
            <p className="text-xl font-bold text-[#F4F4F4] mt-1">
      ...
      <Tabs defaultValue="overview" className="w-full">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData}>
                    ...
                    <XAxis dataKey="month" stroke="#F4F4F4/50" fontSize={12} />
                    <YAxis stroke="#F4F4F4/50" fontSize={12} tickFormatter={(v) => `$${v}`} />
                    <Tooltip 
                      contentStyle={{ 
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#E6A800]/20">
                      <th className="text-left p-3 text-sm font-medium text-[#F4F4F4]/60">Mes</th>
                      <th className="text-right p-3 text-sm font-medium text-[#F4F4F4]/60">Reservas</th>
                      <th className="text-right p-3 text-sm font-medium text-[#F4F4F4]/60">Noches</th>
                      <th className="text-right p-3 text-sm font-medium text-[#F4F4F4]/60">Ingresos</th>
                      <th className="text-right p-3 text-sm font-medium text-[#F4F4F4]/60">Gastos</th>
                      <th className="text-right p-3 text-sm font-medium text-[#F4F4F4]/60">Ganancia</th>
                    </tr>
                  </thead>
                  <tbody>
                    {monthlyData.map((month, idx) => (
                      <tr key={idx} className="border-b border-[#E6A800]/10 hover:bg-[#E6A800]/5">
                        <td className="p-3 text-[#F4F4F4]">{month.fullMonth}</td>
                        <td className="p-3 text-right text-[#F4F4F4]/70">{month.reservations}</td>
                        <td className="p-3 text-right text-[#F4F4F4]/70">{month.nights}</td>
                        <td className="p-3 text-right text-[#E6A800]">{formatCurrency(month.income)}</td>
                        <td className="p-3 text-right text-[#E74C3C]">{formatCurrency(month.expenses)}</td>
                        <td className={`p-3 text-right font-medium ${month.profit >= 0 ? 'text-[#2ECC71]' : 'text-[#E74C3C]'}`}>
              ...
              <CardContent>
                <div className="space-y-4">
                  {platformData.map((platform, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-[#F4F4F4]/5 rounded-lg">
                      <div className="flex items-center gap-3">
                        <div 
                          className="w-4 h-4 rounded-full"
                          style={{ backgroundColor: COLORS[idx % COLORS.length] }}
                        />
                        <span className="text-[#F4F4F4]">{platform.name}</span>
                      </div>
                      <div className="text-right">
                        <p className="font-medium text-[#E6A800]">{formatCurrency(platform.value)}</p>
                        <p className="text-xs text-[#F4F4F4]/50">{platform.count} reservas</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {expenseCategoryData.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-[#F4F4F4]/5 rounded-lg">
                      ...
                      <span className="font-medium text-[#E74C3C]">{formatCurrency(item.value)}</span>
                    </div>
                  ))}
                  {expenseCategoryData.length === 0 && (
                    <p className="text-center text-[#F4F4F4]/50 py-8">No hay gastos registrados</p>
                  )}
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="notes" className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-[#F4F4F4]">Notas y Observaciones</h3>
            <Button
              onClick={() => {
                setEditingNote(null);
                setNoteForm({ title: '', content: '', category: 'general' });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {notes.length === 0 ? (
              ...
              notes.map((note) => (
                <Card key={note.id} className="bg-[#0B0B0B] border-[#E6A800]/20">
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex gap-1">
                        <Button
                          ...
                          className="h-7 w-7 text-[#F4F4F4]/60 hover:text-[#E6A800] hover:bg-[#E6A800]/10"
                        >
                        ...
                        <Button
                          ...
                          className="h-7 w-7 text-[#F4F4F4]/60 hover:text-[#E74C3C] hover:bg-[#E74C3C]/10"
                        >
                          <Trash2 className="w-3 h-3" />
                        </Button>
                      </div>
                    </div>
                    <h4 className="font-medium text-[#F4F4F4] mb-2">{note.title}</h4>
                    <p className="text-sm text-[#F4F4F4]/70 whitespace-pre-wrap">{note.content}</p>
                    <p className="text-xs text-[#F4F4F4]/40 mt-3">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-2 bg-[#F4F4F4]/5 border border-[#E6A800]/20 text-[#F4F4F4] rounded-lg"
              >
                <option value="general" className="bg-[#0B0B0B]">General</option>
                <option value="financial" className="bg-[#0B0B0B]">Financiero</option>
                <option value="maintenance" className="bg-[#0B0B0B]">Mantenimiento</option>
                <option value="guest" className="bg-[#0B0B0B]">Huéspedes</option>
              </select>
            </div>
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="w-full p-3 bg-[#F4F4F4]/5 border border-[#E6A800]/20 text-[#F4F4F4] rounded-lg resize-none"
              />
            </div>
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1 bg-[#E6A800] text-[#0B0B0B] hover:bg-[#E6A800]/90 disabled:opacity-50"
              >
